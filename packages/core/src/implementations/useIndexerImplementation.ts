...
export function useIndexerImplementation<T extends typeof Model> (
  ModelClass: T,
  options?: UseIndexerOptions<T>,
): UseIndexerReturn<T> {
  options = Object.assign({}, defaultOptions, options)
  const indexResource = getImplementation<T, 'index'>('index', options.driver)
  ...
  const index: UseIndexerReturn<T>['index'] = async function index (
    ...
    const orderBy = [...(toValue(options?.orderBy) ?? []), ...(resolvedScopes.orderBy ?? [])]
