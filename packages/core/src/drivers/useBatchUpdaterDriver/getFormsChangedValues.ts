...
import { BatchUpdateForm, UseBatchUpdaterReturn } from '../../contracts/batch-update/UseBatchUpdater'
...
> (
  ...
  const recordChangedValuesOnly: BatchUpdateForm<InstanceType<T>> = {}
  ...
  Object.entries(newRecord).forEach((entry) => {
    const key = entry[0] as keyof BatchUpdateForm<InstanceType<T>>
    const value = entry[1] as BatchUpdateForm<InstanceType<T>>
