...
export interface UseCreateResourceOptions<T extends typeof Model> {
  /**
   ...
   *
   * @example
   * const creator = useCreateResource(Comment, {
   *  merge: () => {
   *    return {
   ...
   */
  onSuccess?: (response: CreateResponse<T>) => void

  /**
   * Callback called when an error occurs (including validation error)
   */
  onError?: (response: CreateResponse<T>, validationErrors?: FormValidationErrors<T>) => void

  /**
   * Callback called when an error occurs (NOT including validation error)
   */
  onStandardError?: (response: CreateResponse<T>) => void
   ...
   */
  onValidationError?: (response: CreateResponse<T>, validationErrors: FormValidationErrors<T>) => void
   ...
   * fails, the record is removed from the store.
   */
   ...
   *
   * @example
   * const strapiPostsCreater = useCreateResources(Post, {
   *   driver: 'strapi'
   * })
   */
  driver?: string
...
export interface UseCreateResourceReturn<T extends typeof Model> {
  /**
  ...
  activeRequests: Ref<Record<string | number, {
    request: Promise<CreateResponse<T>>
    form: PiniaOrmForm<InstanceType<T>>
  }>>
}

export type UseCreateResource<T extends typeof Model> = (
  model: T,
