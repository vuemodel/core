...
const exampleComponents = Object.entries(
  import.meta.glob('./examples/BulkUpdate*.vue', { eager: true }),
)

const tab = useLocalStorage('UseBulkUpdater.tab', '')
</script>
...
<template>
  <div style="display: flex; flex-direction: column; max-width: 900px;">
    <div style="display: flex; flex-direction: row;">
      ...
      >
        {{ exampleComponent[0].replace('./examples/BulkUpdate', '').replace('.vue', '') }}
      </button>
    </div>
